...
such a tab on demand, but if its surface still does not paint the op fails with
`errorCode: "not-painting"`. Check `displayed` and bring the tab to the front with
`showTab` (no focus change) before capturing. `displayed` is a saved-layout fact
...
absent (unknown, not `false`) until the host has reported it.
...
result. A capture op (`screenshot`, `getAccessibilityTree`, `evaluate`, `navigate`)
on a visible tab of a not-in-view workspace mounts the tab on demand under the request
deadline and succeeds with the same `warning`; when the mount cannot happen it fails
with a structured `errorCode` (`workspace-not-visible`, `deadline-exhausted`,
`still-loading`, `navigated-away`, `not-painting`) — see the overview topic.
