...
API:
  ...
  ws.task.updateNoteStatus(noteId, status) → { ok, noteId, status, advisory? }  // Task-note status. Values include `"not_started"`, `"waiting"`, `"discussion_needed"`, `"blocked"`, `"in_progress"`, `"review_required"`, `"complete"`, `"cancelled"`.
    The task's OWN linked agent (assignee / task-note owner) cannot move it out of `"complete"` / `"cancelled"`: the call is a no-op answering the unchanged `status` plus `advisory` (absent when the write went through) — ask the coordinator or user to reopen it.
  ...
  ws.agent.create(name, message, opts?) → { ok, id?, text?, ... }  // Create a sub-agent, or with `topLevel: true` an INDEPENDENT top-level agent. Sub-agent creation starts immediately and auto-subscribes you to its completion events — you are woken when it finishes.
    ...
    Pass `vmResources: { vcpus?, memMib? }` to size a microVM-sandboxed agent's VM (vcpus 1-16, memMib 128-65536); missing fields fall back to the `sandbox.microvm` settings. Accepted and ignored outside microVM workspaces.
...
const SANDBOX_VM_RESOURCES_DOC_LINE: &str = "    Pass `vmResources: { vcpus?, memMib? }` to size a microVM-sandboxed agent's VM (vcpus 1-16, memMib 128-65536); missing fields fall back to the `sandbox.microvm` settings. Accepted and ignored outside microVM workspaces.\n";
        ...
        for is_chief in [false, true] {
            for attention_requests in [true, false] {
                ...
                for (description, includes_details) in [
                    (
                        workspace_api_description(is_chief, &features, false).into_owned(),
                        true,
                    ),
                    (
                        condensed_workspace_api_description(is_chief, &features, false, &[], None),
                        false,
                    ),
                    (
...
"#
    ...
    // `ws.pr.monitor` documents parent takeover in both variants: the
    // adoptable-child rule, the `transferred` notice and the still-working
    // refusal steer (PROTOCOL §5.42 adoption rule 2). The steer must name
    // the monitoring-idle advisory as a retry trigger and must not tell the
    // parent to wait for the child's genuine completion — a child holding
    // only the monitor is adoptable while `ws.agent.watch` defers exactly
    // that completion.
    #[test]
    fn pr_monitor_doc_describes_parent_takeover_in_both_variants() {
        for base in [WORKSPACE_API_DESCRIPTION, WORKSPACE_API_DESCRIPTION_CHIEF] {
            for needle in [
                "PARENT TAKEOVER: a monitor held by your own DIRECT sub-agent",
                "`reason: \"transferred\"` and `adoptedBy`",
                "A child that is still working keeps its monitor",
                "monitoring-idle advisory (`childExternallyWaiting` naming only `waitingOnPrMonitors`)",
                "Do NOT wait for the child's genuine completion",
            ] {
                assert!(base.contains(needle), "missing takeover doc: {needle}");
            }
        }
    }
