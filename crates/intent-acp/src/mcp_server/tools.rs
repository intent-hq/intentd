...
API:
  ...
  ws.agent.send(agentId, message, priority?) → { ok, agentId, delivery?, ... }  // Send a message to another agent. Delivers with interrupt priority by DEFAULT: the target is stopped mid-response and the message is delivered immediately. Pass `priority="queue"` to opt out and queue the message if the target is busy. Success carries `delivery`: `"delivered"` (driving a turn now), `"queued"` (parked in the target's queue), or `"held"` (parked behind the target's pending Q&A). A send refused by the single-pending-message rule returns `ok: false` with `refused: true` and the target's queue echoed.
  ws.agent.sendToTask(taskNoteId, message, priority?) → { ok, taskNoteId, delivery?, ... }  // Follow up with the agent assigned to a task note; more convenient than `send()` when you only know the task note ID. Same interrupt-by-default delivery as `send()`; `priority="queue"` opts out. Same `delivery` outcome and `refused: true` refusal shape as `send()`.
