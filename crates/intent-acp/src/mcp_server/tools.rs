...
API:
  ...
  ws.note.saveAsset({ data, mimeType, originalName? }) → { assetId, path, url }  // Save base64 image or video data as a note asset. A `data:<mime>;base64,` URL prefix is accepted. Embed the returned `url` in markdown.
