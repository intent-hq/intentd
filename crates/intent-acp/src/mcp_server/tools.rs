...
API:
  ...
  ws.agent.send(agentId, message, priority?) → { ok, agentId, delivery?, ... }  // Send a message to another agent. Delivers with interrupt priority by DEFAULT: the target is stopped mid-response and the message is delivered immediately. Pass `priority="queue"` to opt out and queue the message if the target is busy.
    ...
    Only ONE pending message per sender per target: while an earlier message of yours is still in the target's queue, a second send is refused with `ok: false` + `refused: true` + your `pendingMessageId` + the target's current `queue` + an `instruction`. Remediation: keep the pending entry as-is, or `ws.agent.removeQueuedMessage` it and re-send ONE message combining everything — remove + re-send is NOT atomic (the pending entry may deliver in between), and a re-sent message lands at the END of the queue.
  ws.agent.sendToTask(taskNoteId, message, priority?) → { ok, taskNoteId, delivery?, ... }  // Follow up with the agent assigned to a task note; more convenient than `send()` when you only know the task note ID. Same interrupt-by-default delivery as `send()`; `priority="queue"` opts out. Same `delivery` outcomes, single-pending-message rule, and `refused: true` refusal shape as `send()` — a refusal additionally echoes `taskNoteId`.
