...
API:
  ...
  ws.agent.watch(agentId) → { ok, subscriptionId, agentId }  // Watch another agent: you are woken once, at its next completion (it goes idle with an empty pending message queue, fails, or is deleted), and the watch is then retired. Blocker/discussion attention wakes are delivered along the way without ending the watch. Watch again if you care about future turns. A target that goes idle while still owning active background hooks or PR monitors is NOT complete — instead of deferring silently, you get ONE advisory wake per waiting episode (metadata `childExternallyWaiting: true` + `watchStillArmed: false`, with `waitingOnHooks` / `waitingOnPrMonitors` naming them) that CONSUMES the one-shot watch: re-arm `ws.agent.watch` if you still want its genuine completion — the re-armed watch stays silent through further monitoring idles and fires at the real completion/failure/deletion. A watch adopted into an `after_all` delegation group ends at group settlement and cannot be unwatched while grouped (use `agent.cancelSubscriptions` with the groupId). An idle target with nothing pending (no active hooks, PR monitors, event subscriptions, queued messages, outgoing waits, or unresolved blocker/discussion/question, among other waiting reasons) is rejected — it has no future completion; wake it instead (`ws.agent.send` auto-arms a watch on you).
