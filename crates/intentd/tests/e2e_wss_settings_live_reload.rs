...
/// Boot with WSS enabled via config.toml, discover the WSS port + fingerprint via
...
async fn external_edit_live_reloads_and_invalid_edit_keeps_last_good() {
    ...
    // The boot seeded a `[server.wsApi]` section into config.toml to start the
    // WSS listener; every valid external rewrite below must carry it forward
    // or the live-reload would disable the listener and drop our connections.
    let ws_api_section = {
        let text = std::fs::read_to_string(&config_path).expect("read config.toml");
        let idx = text
            .find("[server.wsApi]")
            .expect("wsApi section seeded at boot");
        text[idx..].to_string()
    };
