...
/// Regression e2e (intent-hq/intent#4491): auggie's `tool_call` frame for
/// `workspace_api` carries `title` = the call's `summary`, NO `name`, `kind:
/// 'other'` and `rawInput` = `{code, summary}` verbatim — no tool identifier
/// anywhere. With a prose summary (no `workspace_api` substring) the summary
/// used to be recorded as the tool name, the completion-side FIFO claim gate
/// (`tool_name.contains("workspace_api")`) never fired, and the proposal the
/// JS discarded was never attached. Asserts the `{code, summary}` input-shape
/// rule (name derivation + claim second gate) attaches it.
#[tokio::test]
async fn chief_agent_ws_app_proposal_attached_on_auggie_shaped_tool_call() {
    let Some(script) = gate() else { return };

    let db = std::env::temp_dir().join(format!(
        "intentd-e2e-ws-app-auggie-shape-{}.db",
        uuid::Uuid::new_v4()
    ));
    let store = Store::open(&db).await.expect("open store");
    let bus = EventBus::new(store.clone());
    ...
    // Prose summary: no `workspace_api` substring and no `<name>: ` prefix,
    // so nothing but the `{code, summary}` input shape identifies the tool.
    ...
    // (a) Deterministic attach: the standalone proposal-resource block is
    // persisted, claimed from the binding-time registration through the
    // input-shape gate.
    let standalone = messages.iter().find_map(|msg| {
        msg["contentBlocks"].as_array().and_then(|blocks| {
            blocks.iter().find(|block| {
    ...
    let standalone = standalone.unwrap_or_else(|| {
        panic!(
            "Standalone proposal resource block not attached on auggie-shaped tool_call: {}",
            serde_json::to_string_pretty(&conversation).unwrap()
        )
    });
    assert_eq!(standalone["resource"]["name"], "Update Test Setting");
    ...
    // (b) The persisted tool_use block resolved `workspace_api` from the
    // input shape — NOT the prose title — and still echoes the ACP title.
    let tool_use = messages
        ...
        .unwrap_or_else(|| {
            panic!(
                "No tool_use block in transcript: {}",
                serde_json::to_string_pretty(&conversation).unwrap()
            )
        });
    assert_eq!(
        tool_use["name"],
        "workspace_api",
        "tool_use name must derive from the {{code, summary}} input shape: {}",
        serde_json::to_string_pretty(&conversation).unwrap()
    );
    assert_eq!(tool_use["input"]["_acpTitle"], summary);
    assert_eq!(tool_use["input"]["summary"], summary);
    assert_eq!(tool_use["input"]["code"], js);

    // (c) The tool_result carries NO resource item anywhere — the JS returned
    // a plain object, so the output is just the rendered `{ ok: true }` body.
    let tool_result_has_resource = messages.iter().any(|msg| {
        msg["contentBlocks"].as_array().is_some_and(|blocks| {
            blocks.iter().any(|block| {
                block["type"] == "tool_result"
