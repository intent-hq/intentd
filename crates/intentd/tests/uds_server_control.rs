...
//! toggle works on a UDS-only daemon; that requires a real composition-root daemon
...
/// contract: FE spawns 'serve' (UDS-only), user toggles WS on via UI.
///
...
/// that spawns an actual UDS-only intentd process.
#[tokio::test]
