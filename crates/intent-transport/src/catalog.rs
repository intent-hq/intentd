...
pub(crate) const COLLABORATOR_METHODS: &[(&str, &str)] = &[
    ...
    ("client.hello", "Client boot: binds the connection's logical client id and capabilities. Identity is never taken from it; a non-administrator connection is never an eligible reverse-RPC or tab host regardless of what it advertises."),
    ...
    ("drafts.clear", "Per-client: clears the caller's own draft. Keyed by the connection's client id."),
    ("drafts.get", "Per-client: reads the caller's own draft."),
    ("drafts.set", "Per-client: saves the caller's own draft."),
