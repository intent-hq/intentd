...
use intent_core::{
    ...
    NoteVisibility, Principal, PrincipalId, TaskMetadata, TaskStatus, Workspace, WorkspaceActivity,
    WorkspaceAttention, WorkspaceId, WorkspaceInvite, WorkspaceRole, WorkspaceStatus,
};
use serde_json::json;
use sqlx::Row;
...
// ─── principals / membership / credentials (migration 0122) ────────────────
...
/// Migration 0122 applies cleanly on an existing, populated database: after
/// rewinding to the 0121 schema with two workspaces present, reopening the
...
async fn principals_migration_backfills_existing_workspaces() {
    ...
    let ws_b = WorkspaceId::from("ws-mig-b");
    {
        let store = Store::open(&tmp.path).await.expect("open store");
        for sql in [
            "DELETE FROM _sqlx_migrations WHERE version IN (122, 123)",
    ...
    let store = Store::open(&tmp.path).await.expect("reopen applies 0122");
...
/// (migration `0123`): promoting a second member or adding a second owner
...
/// (intent-hq/intentd#1868). Under the one-owner index (migration `0123`)
...
/// Migration `0123` applies on a database that already holds several owner
...
async fn one_owner_migration_repairs_duplicate_owners_before_indexing() {
    ...
    // Rewind to the pre-0123 shape and seed the duplicates the index forbids.
    ...
    sqlx::raw_sql(include_str!(
        "../migrations/0123_one_owner_per_workspace.sql"
    ))
    .execute(store.write_pool())
    .await
    .expect("0123 repairs the duplicates instead of aborting");
...
/// Migration 0125 keeps the invite link secret next to its hash: a minted
...
async fn workspace_invite_secret_round_trips_and_legacy_rows_read_none() {
    ...
    // A pre-0125 row: inserted without the `secret` column, exactly the shape
    ...
    let legacy = store
        ...
        .expect("get legacy")
        .expect("present");
    assert_eq!(legacy.secret, None, "a pre-0125 row has no stored secret");
