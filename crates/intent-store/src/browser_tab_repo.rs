...
//!
//! `displayed` (intent-hq/intent#4835) is the one host-reported field that is
//! **not** a column: it is a layout fact of the live host — meaningful only
...
/// Process-local `tab_id → displayed` overlay (see the module docs). Shared
/// by every clone of the [`Store`]; an id with no entry reads as `None`.
#[derive(Clone, Default)]
pub(crate) struct DisplayedOverlay(Arc<RwLock<HashMap<String, bool>>>);

impl DisplayedOverlay {
    fn get(&self, tab_id: &str) -> Option<bool> {
        self.0
            .read()
    ...
    /// Remember `tab.displayed` for `tab.tab_id` (a `None` report clears it).
    fn record(&self, tab: &BrowserTab) {
        let mut map = self.0.write().unwrap_or_else(PoisonError::into_inner);
        match tab.displayed {
            Some(displayed) => {
                map.insert(tab.tab_id.clone(), displayed);
            }
            None => {
                map.remove(&tab.tab_id);
            }
        }
    }

    fn forget(&self, tab_id: &str) {
        self.0
            .write()
            .unwrap_or_else(PoisonError::into_inner)
            .remove(tab_id);
...
impl Store {
    ...
    pub async fn list_browser_tabs(&self, id: &WorkspaceId) -> Result<Vec<BrowserTab>> {
        ...
        rows.iter()
            .map(|r| map_row(r, &self.browser_tab_displayed).map(|s| s.tab))
            .collect()
    }
    ...
    async fn get_stored_tab(&self, tab_id: &str) -> Result<Option<StoredTab>> {
        ...
        fetch_stored(&mut conn, tab_id, &self.browser_tab_displayed).await
    }
    ...
    pub async fn upsert_browser_tab(
        ...
        let displayed = self.browser_tab_displayed.clone();
        crate::with_write_txn_retry(|| {
            ...
            async move {
                ...
                let outcome = match fetch_stored(&mut tx, &input.tab_id, &displayed).await? {
                    None => {
                ...
                if !matches!(outcome, BrowserTabUpsertOutcome::Unchanged(_)) {
                    displayed.record(outcome.tab());
                }
                Ok(outcome)
            }
        })
    ...
    pub async fn remove_browser_tab(
        ...
        crate::with_write_txn_retry(|| {
            ...
            async move {
                ...
                let Some(stored) = fetch_stored(&mut tx, tab_id, &displayed).await? else {
                    return Ok(None);
                };
                reject_foreign_host(&stored.tab, host)?;
    ...
    pub async fn close_browser_tab(&self, tab_id: &str) -> Result<Option<BrowserTab>> {
        ...
        crate::with_write_txn_retry(|| {
            ...
            async move {
                ...
                let Some(StoredTab { tab, closed: false }) =
                    fetch_stored(&mut tx, tab_id, &displayed).await?
                else {
                    return Ok(None);
                };
        ...
        crate::with_write_txn_retry(|| {
            ...
            async move {
                ...
                for input in snapshot {
                    ...
                    match fetch_stored(&mut tx, &input.tab_id, &displayed).await? {
                        None => {
                            let tab = new_tab(host, input);
                            write_tab(&mut tx, &tab).await?;
                ...
                for tab in result
                    .opened
                    .iter()
                    .chain(result.updated.iter().map(|(t, _)| t))
                {
                    displayed.record(tab);
                }
                for tab_id in &purged {
                    displayed.forget(tab_id);
                }
                Ok(result)
            }
...
async fn fetch_stored(
    ...
    row.as_ref().map(|r| map_row(r, displayed)).transpose()
}
...
/// Materialize a row, merging in the process-local `displayed` overlay.
fn map_row(r: &SqliteRow, displayed: &DisplayedOverlay) -> Result<StoredTab> {
    ...
    let tab_id: String = r.get("tab_id");
    Ok(StoredTab {
        tab: BrowserTab {
            displayed: displayed.get(&tab_id),
            tab_id,
