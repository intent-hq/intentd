...
//! (multiplayer w1, migration `0122_principals`). Principals are people
...
pub struct WorkspaceAuthorFallback {
    ...
    /// workspace created after migration `0122`.
...
impl Store {
    ...
    /// already has one (exactly one owner per workspace, migration `0123`)
...
/// (`workspace_member_owner_uq`, migration `0123`) — the `(workspace_id,
