...
pub struct WorkspaceSettings {
    ...
    /// sandboxing (requires CoW filesystem support; falls back to worktrees
    /// when unsupported).
...
autoFetch = false
# Copy-on-Write Isolation -- CoW workspaces + per-agent sandboxes; falls back
# to worktrees when unsupported (requires CoW filesystem support).
cowIsolation = false
