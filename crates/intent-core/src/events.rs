...
// Sandbox CoW merge-back outcome (new in intentd; monorepo#1120). Emitted by
// the merge-on-turn-end path when the sandbox commit cannot be fast-merged
// into the workspace worktree: `sandbox:cow:conflict` →
// `{ agentId, sandboxPath, conflictingPaths }`. Sibling of the pre-taxonomy
// `sandbox:cow:created` / `sandbox:cow:merged` emits.
pub const SANDBOX_COW_CONFLICT: &str = "sandbox:cow:conflict";
