...
#[test]
fn codex_probe_launch_keeps_adapter_precedence_and_selects_host_runtime() {
    let cmd = super::codex_probe_launch(
    ...
    let fallback = super::codex_probe_launch(
        None,
        Some(std::path::PathBuf::from("/usr/local/bin/npx")),
        Some(std::path::Path::new("/host/codex")),
    )
    ...
    assert_eq!(
        fallback.program(),
        std::path::Path::new("/usr/local/bin/npx")
    );
    assert_eq!(fallback.env_vars(), cmd.env_vars());
    assert_eq!(fallback.removed_env_vars(), &["CODEX_CONFIG"]);
}

#[test]
fn codex_probe_launch_without_binary_or_npx_is_none() {
    assert!(super::codex_probe_launch(None, None, None).is_none());
}
