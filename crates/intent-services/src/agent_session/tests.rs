...
/// garbled completion echo. The recorded `tool_use` name must be
/// `workspace_api` (derived from the input shape), the summary must still be
/// echoed as `_acpTitle`, and the registered attachment must be claimed.
#[tokio::test]
