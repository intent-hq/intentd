...
pub async fn ensure_extracted_tree(rootfs_path: &Path) -> Result<PathBuf, MicrovmError> {
    ...
    // bsdtar (macOS) and GNU tar both auto-detect xz via -xf. The archive was
    // built by a non-root docker export repack: no device nodes to lose, and
    // ownership lands as the daemon user (libkrun maps the host uid to guest
    // root).
