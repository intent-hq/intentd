...
pub(crate) fn definitions() -> Vec<SettingDefinition> {
    ...
    vec![
        ...
        // Ports the FE `notificationSettings` electron-store bag (fields
        ...
        string(
            ...
            "Desktop-client-local sound path; empty uses the built-in sound",
            "notifications",
            Some(""),
        ),
        boolean(
...
mod tests {
    ...
    /// Ports the FE `notificationSettings` electron-store bag as
    ...
    fn notifications_catalog_entries_match_fe_shape() {
        for (path, expected_type, default) in [
            ("notifications.enabled", "boolean", json!(true)),
            ("notifications.soundEnabled", "boolean", json!(true)),
            ("notifications.soundPath", "string", json!("")),
            (
                "notifications.soundOnlyWhenUnfocused",
                "boolean",
                json!(true),
            ),
            ("notifications.volume", "number", json!(0.5)),
        ] {
            let def = find_definition(path).unwrap_or_else(|| panic!("{path} missing"));
            assert!(!def.sensitive, "{path} must be non-secret");
            assert!(!def.read_only, "{path} must not be read-only");
            ...
            assert_eq!(def.ty.wire_name(), expected_type, "{path}");
