...
- **ws.workspace.setStatusMessage(message)** - Update or clear the work status shown on the workspace card: one plain sentence, ideally under 15 words, naming what is being worked on and where it stands — no counts, check lists, or implementation details
...
5. **Keep workspace status current** - Update `statusMessage` only when high-level work status changes, not for minor implementation details; keep it to one short plain sentence; it is separate from lifecycle and task statuses
