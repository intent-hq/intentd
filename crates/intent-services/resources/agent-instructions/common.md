...
- **Read the response instead of tracking graph state yourself.** Every supplied task comes back with a disposition (`started` / `held:blocked-on-deps` / `held:conflict` / `skipped` / `error`; every non-started row carries a reason), and `unlockPlan` names which held tasks become startable when the running set settles. With effort estimates on the tasks, `unlockPlan.criticalPathMinutes` estimates the serial work remaining.
- **Re-call delegate on settlement wakes.** The app never auto-starts held tasks; when the group-settlement wake arrives, re-call `agent.delegate` with the remaining list — classification is recomputed fresh on every call. A held task whose dependency was cancelled or deleted comes back flagged "decision needed": resolve it with the user rather than re-calling in a loop. A dependency whose agent failed is not flagged — it stays plain `held:blocked-on-deps`, so check the dependency's task status if a hold persists across settlements.
- **Completion wakes name newly unblocked tasks.** When a delegated task's completion makes other tasks startable, the wake message ends with an advisory section — `Tasks now unblocked by this completion: [Title](intent://local/task/{id}) (deps satisfied)` / `(conflict cleared)` — computed fresh when the wake is delivered (not when the child finished), so it reflects the current task state even if the wake sat queued behind your busy turn; several completions delivered together coalesce into one section. Treat it as a hint, not an action: nothing auto-starts, so still re-call `agent.delegate` with the remaining list. An entry annotated "needs attention" (waiting / discussion_needed / blocked / review_required) is unblocked on relations but needs its attention state resolved first.
