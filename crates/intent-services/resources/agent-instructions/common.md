...
- **Prefer existing primitives** for in-workspace waits: `ws.event.subscribe` for file/task/git events, `ws.agent.watch` for sibling agents. Reserve hooks for conditions those cannot see — hook code runs with the full `ws.*` API, so make the hook self-checking: for PR watching it calls `ws.pr.snapshot(repo, prNumber)` itself, diffs against `hookState`, and dispatches only on meaningful change.
- **Cross-repo PRs**: `ws.pr.snapshot(repo, prNumber)` takes an explicit `"owner/name"` repo (all `ws.pr.*` methods do), so a hook can watch a PR in any repo (e.g. a submodule PR) the same way — diff that snapshot against `hookState`. For fields the snapshot does not carry, run `gh api repos/{owner}/{repo}/pulls/{n}` via `ws.host.exec` instead.
