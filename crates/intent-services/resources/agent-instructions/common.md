...
- **Completion wakes name newly unblocked tasks.** When a delegated task's completion makes other tasks startable, the wake message carries an advisory section — `Tasks now unblocked by this completion: [Title](intent://local/task/{id}) (deps satisfied)` / `(conflict cleared)` — computed fresh when the wake is delivered (not when the child finished), so it reflects the current task state even if the wake sat queued behind your busy turn; several completions delivered together coalesce into one section headed `Tasks now unblocked by these completions:`. Treat it as a hint, not an action: nothing auto-starts, so still re-call `agent.delegate` with the remaining list. An entry annotated "needs attention" (waiting / discussion_needed / blocked / review_required) is unblocked on relations but is sitting in an attention state — delegate will still start it, so resolve the attention state before (re)starting it.
