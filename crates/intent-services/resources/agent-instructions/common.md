...
- **Cross-repo PRs**: `ws.pr.*` is scoped to the workspace's own repo. For a PR in a different repo (e.g. a submodule PR), have the hook run `gh api repos/{owner}/{repo}/pulls/{n}` via `ws.host.exec` and diff the fields you care about against `hookState`.
