// Modified by Intent: run the device Codex CLI instead of a bundled dependency.
...
export function startCodexConnection(codexPath?: string, env?: NodeJS.ProcessEnv): CodexConnection {
    ...
    const host = resolveHostCodex(spawnEnv, codexPath);
    const codex = process.platform === 'win32'
        ? spawn(`"${host}" app-server`, { shell: true, env: spawnEnv })
        : spawn(host, ['app-server'], { env: spawnEnv });
