...
//!    and the failure counter like any other installed update
...
impl Supervisor {
        ...
        loop {
            ...
            let mut child = match command.spawn() {
                ...
                Err(e) => {
                    ...
                    if let Some(version) = self.check_after_failed_start(&mut next_check_at).await {
            ...
            loop {
                tokio::select! {
                    status = child.wait() => {
                        ...
                        if let Some(version) =
                            self.check_after_failed_start(&mut next_check_at).await
                        {
                            current_version = version;
                            backoff = self.config.backoff_initial;
                            failures = 0;
    ...
    /// diagnosable). Returns the newly installed version when the channel
    /// published a fix; `None` when already current or the check failed —
    /// the caller then proceeds to its normal backoff/give-up handling.
    async fn check_after_failed_start(&self, next_check_at: &mut Instant) -> Option<String> {
        ...
        *next_check_at = self.schedule_next_check();
        match outcome {
            ...
            Ok(UpdateOutcome::AlreadyCurrent { .. }) => None,
            Err(e) => {
                eprintln!("intentd-sitter: update check failed: {e}");
                None
            }
