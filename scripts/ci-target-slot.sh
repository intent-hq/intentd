...
cmd_purge_candidates() {
  ...
  case "$quiet_min" in ''|*[!0-9]*) echo "purge-candidates: --quiet-min N (minutes, integer >= 1) is required" >&2; usage ;; esac
  [ "$quiet_min" -ge 1 ] || { echo "purge-candidates: --quiet-min must be >= 1" >&2; usage; }
  ...
  while IFS= read -r entry; do
    ...
    # Recursive activity check, evaluated live per candidate: a job writing
    # into the dir keeps it fresh even without a marker (legacy jobs, other
    # repos sharing the root).
